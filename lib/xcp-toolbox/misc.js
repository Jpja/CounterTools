...
function displayNumber(number, numOfDecimals) {
...
//number is rounded if necessary
	...
	formatted = split[0];
	if (numOfDecimals != 0) formatted += "." + split[1];
...
//in rare cases when array lookup and API call fail it will assume divisible asset 
	var indAsset = assetListIndex(asset);
	var divisible = true;
	if (indAsset != -1) {
		if (ASSET_LIST[indAsset][3] == 0) divisible = false;	
	} else {
		//API call
		var jsondata = jsonFromURL("https://counterpartychain.io/api/asset/" + asset);
		if (jsondata != "-1") {
			if(jsondata.hasOwnProperty('divisible')) {
				if (jsondata['divisible'] == 0) divisible = false;
			} else {
