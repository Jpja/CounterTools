...
//in rare cases when array lookup and API call fail it will assume indivisible asset 
//if wrongly assumed indivisble the send amount will be 100,000,000th time smaller, e.g. 0.00000001 XCP instead of 1 XCP
	var indAsset = assetListIndex(asset);
	var divisible = false;
	if (indAsset != -1) {
		if (ASSET_LIST[indAsset][3] != 0) divisible = true;	
	} else {
		//API call
		var jsondata = jsonFromURL("https://counterpartychain.io/api/asset/" + asset);
		if (jsondata != "-1") {
			if(jsondata.hasOwnProperty('divisible')) {
				if (jsondata['divisible'] != 0) divisible = true;
			} else {
