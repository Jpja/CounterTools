...
//             var script = data[i].script;
		...
		if (correct == "yes" && buildTxTime < API_TIMEOUT_MS - 200 && pushTxOK == 0) {
			sendXcpOK = 1;
...
//             var script = data[i].script;
        ...
        if (datachunk_unencoded != "error") {
            ...
            //sendBTCpush(final_trans);  //uncomment to push raw tx to the bitcoin network
            if (pushTxOK == 1) document.getElementById('sendfeedback').innerHTML = "Push Tx OK " + final_trans;
            else document.getElementById('sendfeedback').innerHTML = "Push Tx FAILED " + final_trans;
...
function create_bet_data(betType, deadline, wager, counterwager, expiration, target) {
    ...
    // bet_tx_data = "0000000000000000000000";
