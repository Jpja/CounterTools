...
var sendXcpStatus = "";
var sendTimeouts = [];
function sendXCP_opreturn(add_from, add_to, asset, asset_total, btc_total, transfee, mnemonic, feedbackdiv) {
	hex_byte();    
	...
	//Error tests
	if (getBalance(add_from,asset) < asset_total_input) {
		sendXcpStatus += "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />Address balance is too low.";
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
		return;
	} 
	if (asset_total_input <= 0) {
		sendXcpStatus += "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />Send amount must be a positive number.";
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
		return;
	} 
	if (asset_total_input != asset_total && asset_total_input % 1 !== 0) {
		sendXcpStatus += "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />Asset is indivisible. Therefore the amount must be an integer.";
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
		return;
	}
	if (getBalance(add_from,'BTC') < btc_total+transfee) {
		sendXcpStatus += "<span style=\"color:DarkRed;font-weight:bold;\">CANNOT SEND</span><br />BTC balance is too low.";
		document.getElementById(feedbackdiv).innerHTML = sendXcpStatus;
		return;
	}
	...
	sendTimeouts.push(setTimeout(prepareSendXCP_opreturn_blockchaininfo, 2000, add_from, add_to, asset, asset_total, btc_total, transfee, mnemonic, feedbackdiv));
	sendTimeouts.push(setTimeout(writeSendStatus, 4000, feedbackdiv));        
}
...
var bcTimeouts = [];
function sendBroadcast(add_from, message, value, feefraction, msig_total, transfee, mnemonic, feedbackdiv) {
	hex_byte();    
	bitcore = require('bitcore');
	...
	bcTimeouts.push(setTimeout(prepareBroadcast_bitpay, 1, add_from,  message, value, feefraction, msig_total, transfee, mnemonic, callback, feedbackdiv));
	bcTimeouts.push(setTimeout(prepareBroadcast_blockchaininfo, 2000, add_from,  message, value, feefraction, msig_total, transfee, mnemonic, callback, feedbackdiv));
	bcTimeouts.push(setTimeout(writeBroadcastStatus, 4000, feedbackdiv));
}

function prepareBroadcast_bitpay(add_from,  message, value, feefraction, msig_total, transfee, mnemonic, callback, feedbackdiv) {
...
function prepareBroadcast_blockchaininfo(add_from,  message, value, feefraction, msig_total, transfee, mnemonic, callback, feedbackdiv) {
	if (sendBroadcastOK != 1) { 
